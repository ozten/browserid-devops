...
tar xzvf /vagrant/selenium/jobs.tar.gz -C /var/lib/jenkins/
...
echo "*** Installing node and npm via apt ***" 1>&2
...
echo "*** Fetching and installing 123done ***" 1>&2
rm -rf local-RP
rm -rf mozilla-123done-*
wget -q github.com/mozilla/123done/tarball/local-RP # TODO just until the local RP patch lands in master
tar xzvf local-RP
cd mozilla-123done-*
...
echo "*** Starting 123done and logging to 123done.log ***" 1>&2
